...
SCENARIOS = {
    '🛡️ 기본': {
        'desc': '결품 기준 1주 미만 → 목표 2주 확보. 회전(온라인 6채널)으로 메우고, 부족분은 반응과 재고로 분배(SCM 요청). 이동 상한: 각 채널 현재고의 50%',
...
def calc_results_v20(params_key):
    ...
    for code, d in skus.items():
        ...
        # 분배: 회전 후 잔여 결품을 반응과 재고로 보충(목표 2주, SCM 요청)
        dist, dist_used = calc_distribution(d, moves, CHANNELS, dist_target=2.0)
        after = calc_after_woc(d, moves, CHANNELS)
        combined = {c: moves.get(c, 0) + dist.get(c, 0) for c in CHANNELS}
        rev = calc_expected_revenue(d, combined, CHANNELS, d['price'])  # 회전+분배 통합 회수효과
        mode = '제외' if d.get('locked', False) else '자동회전'
        results.append({'code': code, 'data': d, 'moves': moves, 'dist': dist, 'dist_total': dist_used,
...
def render_scenario(scenario_key, container, allow_slider=False):
    ...
    total_dist = sum(r.get('dist_total', 0) for r in results)
    ...
    if show_only_moved and not search_code:
        filtered = [r for r in filtered if any(v != 0 for v in r['moves'].values()) or r.get('dist_total', 0) > 0]
    if search_code:
    ...
    if _pre_rows:
        ...
        _dist = sum(r.get('dist_total', 0) for r in _base)
    ...
    if rows:
        ...
        styled = (df.style
                  ...
                  .map(mv_color, subset=[('', '분배(반응과→)')]))
        styled = styled.format({('효과', '만원'): '{:,}'.format, ('', '분배(반응과→)'): '{:,}'.format})
    ...
    container.caption(
        '🎨 재고보유주수: 🔴 < 1주  🟡 1~4주  🟢 ≥ 4주   |   '
        '🔁 회전 = 온라인 6채널 간 이동(합계 0)   ·   🟦 분배 = 반응과 재고로 보충(SCM 요청, 목표 2주)   |   '
        '효과 = 회전+분배 결품해소 회수매출(만원)   |   '
        '※ 이동수량은 외부창고(AENS·ADU3·ADQS) 보관분 제외'
    )
    ...
    with col_b2:
        if st.button('📦 분배 요청(SCM)', use_container_width=True, key=f'dist_{scenario_key}'):
            st.info('실 배포 시 반응과→채널 분배 요청 전표/메일 자동 생성 (현재 PoC). 반응과 실재고 연동 후 동작.')
    with col_b3:
        container.caption('실 배포: 회전 승인 → SAP BAPI · 분배 → SCM 요청')
...
def render():
    ...
    tabs = st.tabs(list(SCENARIOS.keys()) + ['📊 채널 별 세부', '📈 실행 효과', '🚫 채널 IN-OUT (MD 기입)', '🔁 리오더 매핑 (SCM 기입)'])
    tab_d, tab_c, tab_ch, tab_fx, tab_x, tab_re = tabs
    with tab_d:
