# -*- coding: utf-8 -*-
"""
v1.4 화면 — 이전 버전 참고용 (반응과 자동분배 모드B 포함 원형)
(페이지 설정·비밀번호 게이트·공통 CSS는 app.py 담당)
"""
import streamlit as st
import pandas as pd

from rebalance_engine_v14 import calc_rebalance, calc_after_woc, calc_expected_revenue
...
def load_data_v14():
    return get_combined_data('v1')


@st.cache_data(show_spinner=False)
def calc_results_v14(params_key):
    skus = load_data_v14()
    params = {
...
def render_scenario(scenario_key, container, allow_slider=False):
    ...
    with st.spinner('계산 중...'):
        ...
        results = calc_results_v14(params_key)
    ...
    with col_f1:
        show_only_moved = st.checkbox(f'이동 발생만', value=True, key=f'moved_v14_{scenario_key}')
    with col_f2:
        mode_filter = st.multiselect('모드', ['A', 'B', '-'], default=['A', 'B'], key=f'mode_v14_{scenario_key}')
    with col_f3:
        sort_by = st.selectbox('정렬', ['온라인 매출 순위 ↑', '기대효과 ↓', '이동수량 ↓', '단품코드'], key=f'sort_v14_{scenario_key}')
    with col_f4:
        hide_locked = st.checkbox('잠금 SKU 숨김', value=False, key=f'lock_v14_{scenario_key}')
    ...
    with col_b1:
        if st.button('✅ 전체 1-클릭 승인', use_container_width=True, type='primary', key=f'approve_v14_{scenario_key}'):
            st.success(f'✓ {moved_count}개 단품 / {total_in:,}장 → SAP BAPI 전송 완료 (mock)')
            st.balloons()
    with col_b2:
        if st.button('✋ Override 화면', use_container_width=True, key=f'override_v14_{scenario_key}'):
...
def render():
    st.markdown('<div class="title-bar">REBA_재고재배치 Agent — 운영 대시보드<span class="ver-badge">v1.4 · 이전 참고용</span></div>', unsafe_allow_html=True)
    last = get_last_update_time()
    col_a, col_b, col_c = st.columns([4, 1, 1])
    with col_a:
        st.caption(f'마지막 갱신: **{last.strftime("%Y-%m-%d %H:%M")}**   |   다음 갱신: 매일 06:00 (Airflow · EHUB 06:00 & 샵링크 06:30 배치 후)   |   8,308 단품 (보수운영.xlsx 기준)')
    with col_b:
    ...
    st.warning('⏪ 이전 버전(v1.4) 참고 화면 — 반응과 자동분배(모드B) 포함 · 리오더 병합/외부창고 분리 미적용. 운영 판단은 v2.0 기준.')
